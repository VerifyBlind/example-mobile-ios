...
1. SDK'ya geri-dönüş URL'i geçilir: `startAuthentication(..., returnUrl: "verifyblinddemo://callback")`
   (`DemoConfig.returnUrl`).
2. `verifyblinddemo` şeması `Config/Info.plist` → `CFBundleURLTypes` altında kayıtlıdır.
...
1. Pass a return URL to the SDK: `startAuthentication(..., returnUrl: "verifyblinddemo://callback")`
   (`DemoConfig.returnUrl`).
2. The `verifyblinddemo` scheme is registered under `CFBundleURLTypes` in `Config/Info.plist`.
